    ...
    const fetchAllowanceTypes = async () => {
      try {
        ...
        if (response.data.success) {
          setAllowanceTypes(response.data.data.allowanceTypes);
        } else {
          setServerError(response.data.message);
        }
      } catch (err) {
        setServerError('Failed to fetch allowance types');
      }
    };
    fetchAllowanceTypes();
  }, [roleApi]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response && err.response.status === 422) {
        ...
        setServerError('Failed to create allowance');
      }
    }
  };
  ...
  return (
    <div className="user-container">
        <div style={{ paddingLeft: '40px', width: '100%', maxWidth: '900px', marginBottom: '24px', textAlign: 'left' }}>
          <h1 className="page-title" style={{ margin: 0, textAlign: 'left' }}>Add New Allowance</h1>
