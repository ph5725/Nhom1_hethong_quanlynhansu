...
const WorkScheduleEdit = () => {
  ...
  // Lấy thông tin work schedule và related data
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [scheduleRes, workSchedulesRes, employeesRes] = await Promise.all([
          axios.get(`${SCHEDULE_API_URL}/${id}`, {
        ...
        if (scheduleRes.data.success) {
          setFormData({
            department_id: scheduleRes.data.data.employee?.department_id || '',
            employee_id: scheduleRes.data.data.employee_id || '',
            shift_id: scheduleRes.data.data.shift_id || '',
            status_id: scheduleRes.data.data.status_id || '',
            work_date: scheduleRes.data.data.work_date || '',
            is_day_off: scheduleRes.data.data.is_day_off || false,
          });
        } else {
          setServerError(scheduleRes.data.message);
        }
  ...
  useEffect(() => {
    if (formData.department_id) {
      ...
      if (!filtered.some((employee) => employee.id === parseInt(formData.employee_id))) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response && err.response.status === 422) {
        ...
        setServerError('Failed to update work schedule');
      }
    }
  };
  ...
  return (
    <div className="user-container">
      <div style={{ paddingLeft: '40px', width: '100%', maxWidth: '900px', marginBottom: '24px', textAlign: 'left' }}>
        <h1 className="page-title" style={{ margin: 0, textAlign: 'left' }}>Edit Work Schedule</h1>
