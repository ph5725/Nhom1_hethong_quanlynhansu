...
const DepartmentEdit = () => {
  ...
  // Lấy thông tin department và employees
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (departmentRes.data.success) {
          setFormData({
            name: departmentRes.data.data.name || '',
            address: departmentRes.data.data.address || '',
            phone: departmentRes.data.data.phone || '',
            manager: departmentRes.data.data.manager?.id || '',
          });
        } else {
          setServerError(departmentRes.data.message);
        }

        if (employeesRes.data.success) {
          setEmployees(employeesRes.data.data.employees); // Adjusted to match controller response
        } else {
  ...
  return (
    <div className="user-container">
      <div style={{ paddingLeft: '40px', width: '100%', maxWidth: '900px', marginBottom: '24px', textAlign: 'left' }}>
        <h1 className="page-title" style={{ margin: 0, textAlign: 'left' }}>Edit Department</h1>
