...
const PositionEdit = () => {
  ...
  useEffect(() => {
    ...
    const fetchPosition = async () => {
      try {
        const API_URL = `http://127.0.0.1:8000/api/${roleApi}/positions`;
        ...
        setServerError('Failed to fetch position');
      }
    };
    fetchPosition();
  }, [id, roleApi]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response && err.response.status === 422) {
        ...
        setServerError('Failed to update position');
      }
    }
  };
  ...
  return (
    <div className="user-container">
      <div style={{ paddingLeft: '40px', width: '100%', maxWidth: '900px', marginBottom: '24px', textAlign: 'left' }}>
        <h1 className="page-title" style={{ margin: 0, textAlign: 'left' }}>Edit Position</h1>
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <div className="form-buttons">
            <Button
              ...
              onClick={() => navigate('/hrm/positions')}
            >
              Cancel
            </Button>
