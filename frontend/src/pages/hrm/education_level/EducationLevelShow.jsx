...
const EducationLevelShow = () => {
  ...
  const handleEdit = (id) => {
    navigate(`/hrm/education-levels/edit/${id}`);
  };
  ...
  return (
    <div className="user-container">
      <div style={{ paddingLeft: '40px', width: '100%', maxWidth: '900px', marginBottom: '24px', textAlign: 'left' }}>
        <h1 className="page-title" style={{ margin: 0, textAlign: 'left' }}>Education Levels</h1>
        {error && <Alert variant="danger" className="error-message">{error}</Alert>}
        <Button
          className="btn-add"
          onClick={() => navigate('/hrm/education-levels/create')}
        >
          Add
        </Button>
      </div>
