...
const ContractEdit = () => {
  ...
  useEffect(() => {
    try {
      ...
      setRoleApi('admin'); // Role mặc định
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [contractRes, listsRes] = await Promise.all([
        ...
        if (contractRes.data.success) {
          setFormData({
            employee_id: contractRes.data.data.employee_id || '',
            type: contractRes.data.data.type || '',
            salary_id: contractRes.data.data.salary_id || '',
            contract_code: contractRes.data.data.contract_code || '',
            start_date: contractRes.data.data.start_date || '',
            end_date: contractRes.data.data.end_date || '',
            sign_date: contractRes.data.data.sign_date || '',
            contract_file: contractRes.data.data.contract_file || '',
            status: contractRes.data.data.status || '',
            note: contractRes.data.data.note || '',
            year_of_service: contractRes.data.data.year_of_service || '',
          });
        } else {
          setServerError(contractRes.data.message);
        }

        if (listsRes.data.success) {
          setEmployees(listsRes.data.data.employees);
          setSalaries(listsRes.data.data.salaries);
          setContractTypes(listsRes.data.data.contractTypes);
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response && err.response.status === 422) {
        ...
        setServerError('Failed to update contract');
      }
    }
  };
  ...
  return (
    <div className="user-container">
        <div style={{ paddingLeft: '40px', width: '100%', maxWidth: '900px', marginBottom: '24px', textAlign: 'left' }}>
          <h1 className="page-title" style={{ margin: 0, textAlign: 'left' }}>Edit Contract</h1>
