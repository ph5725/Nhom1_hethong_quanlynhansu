...
const InsuranceTypeShow = () => {
  ...
  const [roleApi, setRoleApi] = useState(null); // Role dùng cho API
    ...
    const fetchInsuranceTypes = async () => {
      try {
        const API_URL = `http://127.0.0.1:8000/api/${roleApi}/insurance-types`;
        ...
        setError('Failed to fetch insurance types');
      }
    };
    fetchInsuranceTypes();
  }, [roleApi]);
  ...
  const handleEdit = (id) => {
    navigate(`/admin/insurance-types/edit/${id}`);
  };

  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this insurance type?')) {
      try {
        const API_URL = `http://127.0.0.1:8000/api/${roleApi}/insurance-types/${id}`;
        const response = await axios.delete(API_URL, {
          headers: { Authorization: `Bearer ${localStorage.getItem('access_token')}` }
        });
        if (response.data.success) {
          setInsuranceTypes(insuranceTypes.filter((type) => type.id !== id));
        } else {
          setError(response.data.message);
        }
      } catch (err) {
        setError('Failed to delete insurance type');
      }
    }
  };
  ...
  return (
    <div className="user-container">
        <div style={{ paddingLeft: '40px', width: '100%', maxWidth: '900px', marginBottom: '24px', textAlign: 'left' }}>
          <h1 className="page-title" style={{ margin: 0, textAlign: 'left' }}>Insurance Types</h1>
      {error && <Alert variant="danger" className="error-message">{error}</Alert>}
      <Button
        className="btn-add"
        onClick={() => navigate('/admin/insurance-types/create')}
      >
        Add
      </Button>
      </div>
...
export default InsuranceTypeShow;