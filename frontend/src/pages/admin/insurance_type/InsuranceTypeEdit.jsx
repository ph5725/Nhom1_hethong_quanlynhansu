...
const InsuranceTypeEdit = () => {
  ...
  useEffect(() => {
    ...
    const fetchInsuranceType = async () => {
      try {
        ...
        setServerError('Failed to fetch insurance type');
      }
    };
    fetchInsuranceType();
  }, [id, roleApi]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        navigate('/admin/insurance-types');
      } else {
        setServerError(response.data.message);
      }
    } catch (err) {
      if (err.response && err.response.status === 422) {
        ...
        setServerError('Failed to update insurance type');
      }
    }
  };
  ...
  return (
    <div className="user-container">
      <div style={{ paddingLeft: '40px', width: '100%', maxWidth: '900px', marginBottom: '24px', textAlign: 'left' }}>
        <h1 className="page-title" style={{ margin: 0, textAlign: 'left' }}>Edit Insurance Type</h1>
...
export default InsuranceTypeEdit;