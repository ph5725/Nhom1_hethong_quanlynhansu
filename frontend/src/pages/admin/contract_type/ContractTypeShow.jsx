...
const ContractTypeShow = () => {
  ...
  const [roleApi, setRoleApi] = useState(null); // Role dùng cho API
    ...
    const fetchContractTypes = async () => {
      try {
        const API_URL = `http://127.0.0.1:8000/api/${roleApi}/contract-types`;
        ...
        setError('Failed to fetch contract types');
      }
    };
    fetchContractTypes();
  }, [roleApi]);
  ...
  const handleEdit = (id) => {
    navigate(`/admin/contract-types/edit/${id}`);
  };

  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this contract type?')) {
      try {
        const API_URL = `http://127.0.0.1:8000/api/${roleApi}/contract-types/${id}`;
  ...
  return (
    <div className="user-container">
      ...
      <h1 className="page-title" style={{ margin: 0, textAlign: 'left' }}>Contract Types</h1>
      {error && <Alert variant="danger" className="error-message">{error}</Alert>}
      <Button
        className="btn-add"
        onClick={() => navigate('/admin/contract-types/create')}
      >
        Add
      </Button>
      </div>
